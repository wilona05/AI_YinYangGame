...
public class Individual {
    ...
    private int[][] fillBoardRandom() {
        ...
        for (int i = 0; i < this.n; i++) {
            for (int j = 0; j < this.n; j++) {
                ...
                // jika posisi ini di soal adalah '0' (kosong), maka randomize angka pada posisi
                // ini menjadi 1 atau 2 (changeable yin/yang)
                else {
    ...
    // method untuk menghitung fitness board. Nilai fitness yang lebih rendah =
    // lebih baik
    ...
    private void dfs(boolean[][] visited, int currNum, int currRow, int currCol) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // dan angka pada posisi baru sama dengan angka sekarang (fixed & changeable
            // dianggap sama)
            if ((currRow + rowMoves[i] >= 0) && (currRow + rowMoves[i] < n) && (currCol + colMoves[i] >= 0)
                    && (currCol + colMoves[i] < n)
                    && (currNum == board[newRow][newCol] || currNum * -1 == board[newRow][newCol])
